...
@test "($PLUGIN_COMMAND_PREFIX:info) success" {
  ...
  assert_contains "${lines[*]}" "typesense://:$password@dokku-typesense-l:8108"
}
...
@test "($PLUGIN_COMMAND_PREFIX:info) replaces underscores by dash in hostname" {
  ...
  assert_contains "${lines[*]}" "typesense://:$password@dokku-typesense-test-with-underscores:8108"
...
@test "($PLUGIN_COMMAND_PREFIX:info) success with flag" {
  ...
  password="$(sudo cat "$PLUGIN_DATA_ROOT/l/PASSWORD")"
  assert_output "typesense://:$password@dokku-typesense-l:8108"
