...
@test "($PLUGIN_COMMAND_PREFIX:link) exports TYPESENSE_URL to app" {
  ...
  assert_contains "$url" "typesense://:$password@dokku-typesense-ls:8108"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.typesense.ls:dokku-typesense-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
...
@test "($PLUGIN_COMMAND_PREFIX:link) uses apps TYPESENSE_DATABASE_SCHEME variable" {
  ...
  url=$(dokku config:get my-app TYPESENSE_URL)
  password="$(sudo cat "$PLUGIN_DATA_ROOT/ls/PASSWORD")"
  assert_contains "$url" "typesense2://:$password@dokku-typesense-ls:8108"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
